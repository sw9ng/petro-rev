...
export const CompanyCashManagement = () => {
  ...
  const handleAddIncomeInvoice = async () => {
    ...
    const { error } = await addIncomeInvoice(selectedCompany, invoiceData);
  ...
  const handleAddExpenseInvoice = async () => {
    ...
    const { error } = await addExpenseInvoice(selectedCompany, invoiceData);
  ...
  const companyIncomeInvoices = incomeInvoices.filter(invoice => invoice.company_id === selectedCompany);
  const companyExpenseInvoices = expenseInvoices.filter(invoice => invoice.company_id === selectedCompany);
    ...
    <div className="space-y-6">
      ...
      {selectedCompany && (
        <>
          ...
          {/* Accounts List */}
          {selectedCompanyData && selectedCompanyData.accounts && selectedCompanyData.accounts.length > 0 && (
            <Card>
              <CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {selectedCompanyData.accounts.map((account) => (
