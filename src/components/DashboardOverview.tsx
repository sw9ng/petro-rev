...
import { formatNumber, formatCurrency, formatDateForDisplay } from '@/lib/numberUtils';
...
export const DashboardOverview = () => {
  ...
  // Calculate daily stats using local time
  ...
  // Calculate today's fuel sales using local time
  const todayFuelSales = fuelSales.filter(sale => {
    const saleDate = formatDateForDisplay(sale.sale_time);
            ...
            {latestShift ? (
              <div className="space-y-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {format(formatDateForDisplay(latestShift.start_time), "PPPp", { locale: tr })}
                </div>
